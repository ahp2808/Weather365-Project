...
const getWeather = (c) => {
  ...
  fetch(Api_Link + c + API_Key + "&units=metric")
    ...
    .then((response) => {
      ...
      frm1.value = response.city.name;
      frm2.value = response.list[3].dt_txt.slice(0, 10);
      frm3.value = response.list[3].main.temp+"C";
      frm4.value = response.list[3].main.temp_max+"C";
      frm5.value = response.list[3].main.temp_min+"C";
      frm6.value = response.list[3].main.humidity+"%";
      frm7.value = response.list[3].wind.speed+"m/s";
    })
    .catch((err) => {
      console.error(err);
